...
import React, { useState, useEffect, useMemo, useRef } from 'react'; // Ajout de useRef pour la carte
import { MapContainer, TileLayer, Marker, Popup } from 'react-leaflet';
import { Share2, Zap, List, Map as MapIcon, Navigation, Lock, MapPin, AlertCircle, Info, Edit, Search, XCircle, CheckCircle, User, FileText } from 'lucide-react';
...
function App() {
  ...
  // --- OPTIMISATION 1: Timer Dynamique ---
  const [currentTime, setCurrentTime] = useState(Date.now());
  
  useEffect(() => {
    // Met à jour l'heure toutes les secondes pour faire avancer le timer
    const timer = setInterval(() => setCurrentTime(Date.now()), 1000);
    return () => clearInterval(timer);
  }, []);
  
  // --- OPTIMISATION 2: Ref pour la carte (Fermeture Popup) ---
  ...
  const sendReport = async (status) => {
    ...
    if (!error) {
      ...
      // --- OPTIMISATION 2 : Fermer le popup après succès ---
      if (mapRef.current) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 font-sans text-slate-900 pb-24 relative">
      
      {/* Affichage conditionnel du Toast */}
      {toast && <Toast message={toast.message} type={toast.type} onClose={() => setToast(null)} />}

      <header className="bg-gradient-to-r from-slate-900 to-slate-800 text-white p-4 sticky top-0 z-[1000] flex justify-between items-center shadow-2xl border-b-2 border-yellow-500">
      ...
      <main className="max-w-2xl mx-auto p-4 space-y-5">
        ...
        <section className="bg-white rounded-3xl p-6 shadow-xl border-2 border-slate-200 relative overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              ...
              disabled={loading}
              className="bg-gradient-to-br from-slate-900 to-slate-700 text-white p-3 rounded-2xl shadow-lg hover:shadow-xl active:scale-95 transition-all disabled:opacity-50"
              ...
              <div className="flex gap-2">
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    if (quartierInfos) setUserCoords({ lat: quartierInfos.lat, lng: quartierInfos.lng });
                    setShowManualSelect(false);
                  }}
          ...
          {/* Badge visuel pour le timer (Utilisation de currentTime) */}
          {!canVote(selectedQuartier) && selectedQuartier && (
            <div className="bg-orange-50 border border-orange-200 text-orange-700 text-[10px] font-bold text-center py-2 mt-3 rounded-full flex items-center justify-center gap-2 animate-pulse">
               <Lock size={12} /> 
               {/* Utilisation de currentTime pour que le timer descende en temps réel */}
               Vote bloqué pour {Math.ceil((30 * 60 * 1000 - (currentTime - userVotes[selectedQuartier])) / 60000)} min
            </div>
          )}
        </section>
        ...
        {/* --- VUE CARTE (AVEC REF ET POPUP AMÉLIORÉ) --- */}
        {activeTab === 'map' && (
          <div className="rounded-3xl overflow-hidden shadow-2xl h-[500px] border-4 border-white">
            <MapContainer center={[4.05, 9.70]} zoom={12} className="h-full w-full" ref={mapRef}>
              <TileLayer 
              ...
              {Object.entries(neighborhoodStats).map(([neighborhood, stats]) => {
                ...
                // Récupérer le dernier signalement pour l'heure
                const latestReport = [...stats.reports].sort((a, b) => 
                  ...
                  >
                    <Popup>
                      <div className="p-3 min-w-[200px] font-sans">
                        <p className="font-black text-sm uppercase text-slate-800 mb-2 border-b pb-1 text-center">
                          {neighborhood}
                        </p>
                        
                        <div className="flex flex-col gap-2 my-2">
                          <div className="flex justify-between items-center bg-slate-50 p-1.5 rounded">
                            <span className="text-[10px] text-slate-500 uppercase font-bold">Dernier avis:</span>
                          ...
                          <div className="flex gap-2 justify-center">
                            <span className="bg-red-100 text-red-700 px-2 py-1 rounded text-[10px] font-bold flex items-center gap-1">
                              🛑 {stats.down}
                            </span>
                        ...
                        {/* Boutons de vote rapide direct sur la carte */}
                        <div className="grid grid-cols-2 gap-1 mt-2">
                          <button 
                            onClick={() => { setSelectedQuartier(neighborhood); sendReport('down'); }}
                            disabled={!canVote(neighborhood)}
                            className="bg-red-600 text-white text-[9px] py-1.5 rounded font-bold hover:bg-red-700 disabled:opacity-50 transition"
                          >
                            COUPÉ
                          </button>
                          <button 
                            onClick={() => { setSelectedQuartier(neighborhood); sendReport('up'); }}
                            disabled={!canVote(neighborhood)}
                            className="bg-green-600 text-white text-[9px] py-1.5 rounded font-bold hover:bg-green-700 disabled:opacity-50 transition"
                          >
                            RÉTABLI
                          </button>
                        </div>
                        
                        {!canVote(neighborhood) && (
                          <p className="text-[8px] text-orange-600 text-center mt-1 font-bold italic">
                            {/* Timer aussi dynamique dans le popup si besoin, ici statique pour simplicité */}
                            Vote bloqué (30 min)
                          </p>
                        )}
                      </div>
        ...
        {activeTab === 'list' && (
          <div className="space-y-3">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
              ...
              <button 
                onClick={() => { setStatusFilter('down'); setSearchQuery(''); }} 
                className={`px-3 py-1.5 rounded-full text-[10px] font-bold border transition whitespace-nowrap ${
                  statusFilter === 'down' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-red-600 border-red-200 hover:bg-red-50'
                }`}
              >
                Coupés seulement
              </button>
            ...
            {Object.entries(neighborhoodStats)
              ...
              .filter(([_, stats]) => { if (statusFilter === 'down') return stats.down > stats.up; return true; })
              .sort((a, b) => {
              ...
              .map(([neighborhood, stats]) => {
                ...
                return (
                  <div key={neighborhood} className={`bg-white p-4 rounded-2xl flex justify-between items-center shadow-md border-l-4 transition hover:shadow-lg ${isDown ? 'border-red-600' : 'border-green-600'}`}>
                    ...
                    <div className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase ${isDown ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                      {isDown ? 'COUPÉ' : 'OK'}
                    </div>
                  </div>
                );
      ...
      <footer className="fixed bottom-4 left-4 z-50">
        <button 
          ...
          className="bg-gradient-to-r from-green-500 to-green-600 text-white px-5 py-3 rounded-full font-black shadow-xl shadow-green-500/30 flex items-center gap-2 text-xs hover:scale-105 transition-transform active:scale-95 border-b-4 border-green-700"
        >
        ...
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-slide-up">
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  ...
                  Si vous avez des questions ou des réclamations, veuillez nous contacter au <strong>+237 620 187 495</strong>.
                </p>
              </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100">
              <button onClick={() => setShowLegal(false)} className="w-full bg-slate-900 text-white py-3 rounded-xl font-black text-xs hover:bg-slate-800 transition shadow-lg">
                J'ai compris
              </button>
            </div>
          </div>
      ...
      <style>{`
        ...
        @keyframes bounceIn { 0% { transform: translate(-50%, -100%); opacity: 0; } 50% { transform: translate(-50%, 10%); opacity: 1; } 70% { transform: translate(-50%, -5%); } 100% { transform: translate(-50%, 0); opacity: 1; } }
